...
function getParams(){
	...
	var strid="'yhbh':'"+localStorage.userid+"'";
	
	var jsonParamStr="{"+formParam+strPageS+strPageN+strAcc+strid+"}";
	return jsonParamStr;
}
...
function listHandleTasks(){
	...
	jsonp({
		url:localStorage.Rooturl+'MobileAction!mjxz.action',
		data:{
			callback:'getTasks',
...
function getTasks(obj){
	...
	localStorage.stateModel = obj.state;
...
$('#box').delegate('.show_list','tap',function(){
	...
	//localStorage.usrI=$('#account').val();
	//console.log($(this).data('adr'))
	location.href='CostReductionCon.html';			
});
//		
